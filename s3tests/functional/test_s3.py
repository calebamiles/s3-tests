...
def _get_post_url(bucket, conn=None):
    if not conn:
	conn = s3.main 
...
def _get_post_policy(bucket, key, expires=6000, acl='private', \
		min_size=0,max_size=1024, content_type='text/plain', \
		redirect_url=None, headers=None):

    utc = pytz.utc
    expiration = datetime.datetime.now(utc)
    expiration += datetime.timedelta(seconds=+expires)

    conditions = []
    conditions.append({'bucket' : bucket.name})
    conditions.append(['starts-with', '$key', key])
    conditions.append({'acl' : acl})
    conditions.append(['starts-with', '$Content-Type', content_type])
    if (redirect_url):
	conditions.append(['eq', '$success_action_redirect', redirect_url])

    if (headers):
        for header in headers:
             conditions.append(["starts-with", header,  headers[header]])
    ...
    encoded_policy = base64.b64encode(json_policy)

    return encoded_policy


def _get_post_payload(bucket, key, value, policy=None, conn=None, \
	    acl='private', success_code=None, content_type='text/plain', \
            redirect_url=None, headers=None):
    ...
    payload['acl'] = acl
    if (policy):
	access_key = conn.aws_access_key_id
	secret_key = conn.aws_secret_access_key
	sig = base64.b64encode(hmac.new(secret_key, policy, sha).digest())
    ...
    if (success_code):
	 payload['success_action_status'] = success_code

    if (redirect_url):
        payload['success_action_redirect'] = redirect_url
...
def _make_post_request(bucket, key, value_string, acl='private', conn=None, \
           content_type='text/plain', signed=True, \
           success_code=None, redirect_url=None, min_size=0, \
           max_size=1024, expires=6000, headers=None):

    if not conn:
        conn = s3.main

    url = _get_post_url(bucket, conn)

    policy = None
    if (signed):
        policy = _get_post_policy(bucket, key, expires=expires, acl=acl, \
                             min_size=min_size, max_size=max_size, \
                             content_type=content_type, \
                             redirect_url=redirect_url, headers=headers)

    payload = _get_post_payload(bucket, key, value_string, policy=policy, \
                conn=conn, acl=acl, success_code=success_code, \
                content_type=content_type, redirect_url=redirect_url, \
                headers=headers)
...
def test_post_object_anonymous_request():
    ...
    r = _make_post_request(bucket, key_name, value_string, acl=acl, signed=False)
...
def test_post_object_authenticated_request():
    ...
    r = _make_post_request(bucket, key_name, value_string)
    eq(r.status_code, 204)
    key = bucket.get_key(key_name)
    got = key.get_contents_as_string()
    eq(got, value_string)
...
def test_post_object_set_success_code():
    ...
    r = _make_post_request(bucket, key_name, value_string, acl=acl, \
               success_code=success_code, signed=False)

    eq(r.status_code, 201)
    message = ET.fromstring(r.content).find('Key')
    eq(message.text, key_name)
...
def test_post_object_set_key_from_filename():
    ...
    key_name = 'foo.txt'
    value_string = 'bar'
    value = (key_name, value_string) # interpreted as a file/content in the request

    url = _get_post_url(bucket)
    policy = _get_post_policy(bucket, key_name) 
    payload = _get_post_payload(bucket, "${filename}", value, policy=policy) 
...
def test_post_object_ignored_header():
    ...
    key_name = 'foo.txt'
    value_string = 'bar'
    headers = {'x-ignore-foo' : 'bar'}

    url = _get_post_url(bucket)
    policy = _get_post_policy(bucket, key_name) 
    payload = _get_post_payload(bucket, key_name, value_string, \
...
def test_post_object_success_redirect_action():
    ...
    key_name = 'foo.txt'
    value_string = 'foo'
    redirect_url = _get_post_url(bucket)
    ...
    key = bucket.get_key(key_name)
    eq(url,
	'{rurl}?bucket={bucket}&key={key}&etag=%22{etag}%22'.format(\
                    rurl = redirect_url, bucket = bucket.name, \
...
def test_post_object_invalid_signature():
    ...
    url = _get_post_url(bucket)
    policy = _get_post_policy(bucket, key_name) 
    payload = _get_post_payload(bucket, key_name, value_string, policy=policy)
...
def test_post_object_invalid_access_key():
    ...
    policy = _get_post_policy(bucket, key_name) 
    payload = _get_post_payload(bucket, key_name, value_string, policy=policy)
    payload['AWSAccessKeyId'] = payload['AWSAccessKeyId'][::-1]
